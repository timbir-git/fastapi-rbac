...
from models import MenuModel, RoleMenuModel, RoleModel, UserModel, UserRoleModel, CertificateModel
...
CertificateDao = DbHelper(CertificateModel)
...
async def has_roles(uid):
    ...
    sql = """select r.id, r.name, ur.status from sys_role as r , sys_user_role as ur where r.id = ur.rid and
             ur.uid = %s and r.status = 1  and ur.status !=9 order by ur.status desc
            """
